...
def firstFit(process,mem_size):
	...
	for p in process:
		...
		for i in range(0,len(memBuffer)):
			q = memBuffer[i]
			if alloted==0 and q[0]==0 and q[1]>=p.size:
				alloted=1
				tempBuffer.append([p.pid,p.size])
				if q[1]>p.size: tempBuffer.append([0,q[1]-p.size])
			elif alloted==1 or q[0]!=0 or (q[0]==0 and q[1]<p.size):
				tempBuffer.append(q)
			
			if alloted==0 and q[0]==0: free+=q[1]
			
		if alloted==1: success+=1
		reached+=1
		if alloted==0 and free>=p.size: externalFragmentation+=p.size
		memBuffer = tempBuffer
